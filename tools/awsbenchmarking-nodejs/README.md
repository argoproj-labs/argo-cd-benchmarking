...
1. Create n number of EKS clusters with 1 of them being the ArgoCD cluster and all others being the App clusters. 
    1. The clusters will be named: argotesting#. The ArgoCD cluster is always argotesting0. 
...
2. You can specify the instance type for the ArgoCD cluster’s nodegroup. The App cluster’s nodegroups are set to utilize m5.large.
3. The ArgoCD cluster will have the following installed on it:
    1. ArgoCD
    2. Prometheus/Grafana
    3. aws-load-balancer-controller
4. The following actions are available through the script:
    1. create - create n number of EKS clusters. Can be used to add clusters to an existing fleet. When first ran, the first cluster will be reserved as the ArgoCD cluster. It will also install Prometheus/Grafana, and the aws-load-balancer-controller into this cluster. 
    2. delete - delete n number of EKS clusters.
    3. installArgo - will install ArgoCD on the ArgoCD cluster (first cluster).
    4. scaleApps - adding or subtracting the number of apps per app cluster
    5. deleteAllApps - deletes all apps on all app clusters.
    6. scaleArgoCluster - change the number of nodes or the instance type of the ArgoCD cluster.
    7. scaleAppClusters - change the number of nodes on the App clusters.
    8. scaleArgoController - change the number of shards of the ArgoCD Application Controller
    9. scaleArgoServer - change the number of replicas of the ArgoCD API Server
    10. scaleArgoRepoServer - change the number of replicas of the ArgoCD Repo Server
    11. loginArgo - login to ArgoCD
    12. scalePrometheus - change the number of replicas of the Prometheus server
    13. setArgoControllerProc - change the number of status/operations processors on the ArgoCD Application Controller
    14. setArgoControllerRecTimeout - change the ArgoCD Application Controller resync policy timeout.
    15. setArgoControllerLogLevel - change the ArgoCD Application Controller log level.
    16. setArgoControllerShardAlgorithm - change the ArgoCD Application Controller sharding algorithm.
...
5. Add some apps to the app cluster:
...
6. Open your Grafana dashboard and open the provided ArgoCD dashboard.

![Image: image.png](images/image1.png)

7. You can view the ArgoCD dashboard when performing your testing.

![Image: image.png](images/image2.png)

8. Now lets add some more clusters. The following command will create 9 more clusters as originally you created 2 already.

```
node index.js --action create --numClusters 11
```

9. Scale the apps across the new clusters. Note I changed the numAppsPerCluster to 1000 and with 10 App clusters I will have a total of 10000 apps. This might take awhile.
...
10. Checking Grafana, you should see 10000 apps.

![Image: image.png](images/image3.png)

11. Lets push a git change to the repo. Clone the repo you forked and modify the configmap2kb/configmap.yaml. You can just add or change a few characters in the randomstring key.
12. Commit the change and push it to your repo in Github.
13. You should now see ArgoCD syncing all 10000 apps. You see this from the amount of Out-of-Sync apps and the Workqueue depth:

![Image: image.png](images/image4.png)
![Image: image.png](images/image5.png)

14. In order to support sharding our ArgoCD application controller, we will change the instance type of our ArgoCD cluster:

```
node index.js --action scaleArgoCluster --instanceType m5.4xlarge --numNodes 1
```

15. Now we will shard the ArgoCD application controller by 10.

```
node index.js --action scaleArgoController --numReplicas 10
```

16. Perform another git change and compare the results in Grafana. With sharding set to 10, we see the sync time drop from 42 mins to 17 mins.
